...
`lantern.models.site.SiteEntry` and `lantern.models.site.SiteContent`

Static site entries and content represent pages or other files within a [Site](/docs/architecture.md#sites) generated
by [Outputs](/docs/architecture.md#outputs) for use by [Exporters](/docs/architecture.md#exporters).

`SiteEntry` instances hold properties of a file details about a file without its contents. `SiteContent` instances
inherit from `SiteEntry` and include a text or binary value for a file's contents.

> [!TIP]
> `SiteEntry`'s are useful where only the structure of the site, or file properties such as media type, are needed,
> such as calculating resource paths for cache invalidation.

`SiteEntry` instances hold properties for
