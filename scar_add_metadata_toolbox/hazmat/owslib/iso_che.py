            ...
            for extent in extents:
                if val is None:
                    for e_ in extent.findall(util.nspath_eval("gmd:EX_Extent/gmd:geographicElement", namespaces)):
                        if (
                            e_.find(util.nspath_eval("gmd:EX_GeographicBoundingBox", namespaces)) is not None
                            or e_.find(util.nspath_eval("gmd:EX_BoundingPolygon", namespaces)) is not None
                        ):
...
class MD_Distributor(object):
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            self.contact = None
            val = md.find(
                util.nspath_eval("gmd:MD_Distributor/gmd:distributorContact/che:CHE_CI_ResponsibleParty", namespaces)
            )
            if val is not None:
