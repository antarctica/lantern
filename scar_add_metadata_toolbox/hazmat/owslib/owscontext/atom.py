...
from scar_add_metadata_toolbox.hazmat.owslib.util import nspath_eval, element_to_string

from scar_add_metadata_toolbox.hazmat.owslib.util import log

from scar_add_metadata_toolbox.hazmat.owslib.owscontext.common import is_empty, extract_p, try_int, try_float
...
def ns_elem(ns_prefix, elem_name):
    ...
    if ns_uri is not None:
        return """{%(ns_uri)s}%(elem_name)s""" % {"ns_uri": ns_uri, "elem_name": elem_name}
...
def parse_owc_content(content_node):
    ...
    content_dict = {"type": mimetype, "url": url, "content": str(child_elem), "title": title}
    return content_dict
...
def parse_entry(entry_node):
    ...
    for val in vals:
        ...
        link = {"href": href, "type": mimetype, "length": length, "lang": lang, "title": title, "rel": rel}
        ...
        if link.get("rel") == "alternate" and not is_empty(link):
            ...
            log.warn("unknown link type in Ows Resource entry section: %r", link)

    resource_base_dict["properties"]["links"].update({"alternates": links_alternates})
    resource_base_dict["properties"]["links"].update({"previews": links_previews})
    ...
    for offering_node in offering_nodes:
        ...
        contents = []
        styles = []
        operation_nodes = offering_node.findall(util.nspath_eval("owc:operation", ns))
        ...
        content_nodes = offering_node.findall(util.nspath_eval("owc:content", ns))
        ...
        style_nodes = offering_node.findall(util.nspath_eval("owc:styleSet", ns))
        ...
        offering_dict = {"code": offering_code, "operations": operations, "contents": contents, "styles": styles}
    ...
    resource_base_dict["properties"].update({"minscaledenominator": min_scale_denominator})
    ...
    resource_base_dict["properties"].update({"maxscaledenominator": max_scale_denominator})
...
def decode_atomxml(xml_string):
    ...
    lang = util.testXMLAttribute(feed_root, "{http://www.w3.org/XML/1998/namespace}lang")
    ...
    context_base_dict["properties"]["links"].update({"profiles": links_profile})
    ...
    val_pixel_width = None if val_display is None else val_display.find(util.nspath_eval("owc:pixelWidth", ns))
    val_pixel_height = None if val_display is None else val_display.find(util.nspath_eval("owc:pixelHeight", ns))
    val_mm_per_pixel = None if val_display is None else val_display.find(util.nspath_eval("owc:mmPerPixel", ns))
    ...
    owc_display = {"pixelWidth": pixel_width, "pixelHeight": pixel_height, "mmPerPixel": mm_per_pixel}
...
def axml_context(d):
    ...
    spec_reference = [axml_link(do) for do in extract_p("properties.links.profiles", d, [])]
    ...
    context_metadata = [axml_link(do) for do in extract_p("properties.links.via", d, [])]
    ...
    keywords = [axml_category(do) for do in extract_p("properties.categories", d, [])]
...
def axml_resource(d):
    ...
    resource_metadata = [axml_link(do) for do in extract_p("properties.links.via", d, [])]
    [entry.append(el) for el in resource_metadata if el is not None]

    content_description = [axml_content(do) for do in extract_p("properties.links.alternates", d, [])]
    [entry.append(el) for el in content_description if el is not None]

    preview = [axml_link(do) for do in extract_p("properties.links.preview", d, [])]
    [entry.append(el) for el in preview if el is not None]

    content_by_ref = [axml_link(do) for do in extract_p("properties.links.data", d, [])]
    [entry.append(el) for el in content_by_ref if el is not None]

    offerings = [axml_offering(do) for do in extract_p("properties.offerings", d, [])]
    ...
    if min_scale_denominator is not None:
        etree.SubElement(entry, ns_elem("owc", "minScaleDenominator")).text = str(min_scale_denominator)

    max_scale_denominator = try_float(extract_p("properties.maxscaledenominator", d, None))
    # <owc:maxScaleDenominator>25000</owc:maxScaleDenominator>
    if max_scale_denominator is not None:
        etree.SubElement(entry, ns_elem("owc", "maxScaleDenominator")).text = str(max_scale_denominator)
    ...
    if is_empty(d):
        ...
        try:
            ...
            if pixel_width is not None:
                etree.SubElement(creator_display, ns_elem("owc", "pixelWidth")).text = str(pixel_width)
            pixel_height = try_int(extract_p("pixelHeight", d, None))
            if pixel_height is not None:
                etree.SubElement(creator_display, ns_elem("owc", "pixelHeight")).text = str(pixel_height)
            mm_per_pixel = try_float(extract_p("mmPerPixel", d, None))
            if mm_per_pixel is not None:
                etree.SubElement(creator_display, ns_elem("owc", "mmPerPixel")).text = str(mm_per_pixel)
...
def axml_offering(d):
    ...
    if is_empty(d):
        ...
        try:
            ...
            operations = [axml_operation(do) for do in extract_p("operations", d, [])]
            [offering.append(el) for el in operations if el is not None]
            # use axml_content here
            contents = [axml_content(do) for do in extract_p("contents", d, [])]
