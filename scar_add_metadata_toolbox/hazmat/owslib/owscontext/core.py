...
from scar_add_metadata_toolbox.hazmat.owslib.owscontext.common import try_float, try_int, extract_p, build_from_xp
    ...
    * + extension :Any [0..*]
    ...
    def __init__(
        ...
        if len(self.spec_reference) <= 0:
            self.spec_reference.append(OwcLink(href=GENERIC_OWCSPEC_URL, rel="profile"))
    ...
    def to_dict(self):
        ...
        return {
            ...
            "properties": {
                ...
                "date": None if self.time_interval_of_interest is None else self.time_interval_of_interest.__str__(),
                "rights": self.rights,
                "authors": [] if len(self.authors) <= 0 else [obj.to_dict() for obj in self.authors],
                "publisher": self.publisher,
                "display": None if self.creator_display is None else self.creator_display.to_dict(),
                "generator": None if self.creator_application is None else self.creator_application.to_dict(),
                "categories": [] if len(self.keywords) <= 0 else [obj.to_dict() for obj in self.keywords],
                "links": {
                    "profiles": [] if len(self.spec_reference) <= 0 else [obj.to_dict() for obj in self.spec_reference],
                    "via": [] if len(self.context_metadata) <= 0 else [obj.to_dict() for obj in self.context_metadata],
                },
            },
            "features": [] if len(self.resources) <= 0 else [obj.to_dict() for obj in self.resources],
        }
    ...
    def from_dict(cls, d):
        ...
        return OwcContext(
            id=d["id"],
            spec_reference=[OwcLink.from_dict(do) for do in extract_p("properties.links.profiles", d, [])],
            area_of_interest=extract_p("bbox", d, None),
            context_metadata=[OwcLink.from_dict(do) for do in extract_p("properties.links.via", d, [])],
            ...
            authors=[OwcAuthor.from_dict(do) for do in extract_p("properties.authors", d, [])],
            ...
            time_interval_of_interest=TimeIntervalFormat.from_string(extract_p("properties.date", d, None)),
            keywords=[OwcCategory.from_dict(do) for do in extract_p("properties.categories", d, [])],
    ...
    * + extension :Any [0..*]
    ...
    def to_dict(self):
        ...
        return {
            ...
            "properties": {
                ...
                "abstract": self.subtitle,
                "updated": self.update_date,
                "date": None if self.temporal_extent is None else self.temporal_extent.__str__(),
                "authors": [] if len(self.authors) <= 0 else [obj.to_dict() for obj in self.authors],
                "publisher": self.publisher,
                "rights": self.rights,
                "categories": [] if len(self.keywords) <= 0 else [obj.to_dict() for obj in self.keywords],
                "links": {
                    ...
                    else [obj.to_dict() for obj in self.content_description],
                    "previews": [] if len(self.preview) <= 0 else [obj.to_dict() for obj in self.preview],
                    "data": [] if len(self.content_by_ref) <= 0 else [obj.to_dict() for obj in self.content_by_ref],
                    "via": []
                    if len(self.resource_metadata) <= 0
                    else [obj.to_dict() for obj in self.resource_metadata],
                },
                "offerings": [] if len(self.offerings) <= 0 else [obj.to_dict() for obj in self.offerings],
                "active": self.active,
    ...
    def from_dict(cls, d):
        return OwcResource(
            ...
            publisher=extract_p("properties.publisher", d, None),
            rights=extract_p("properties.rights", d, None),
            temporal_extent=TimeIntervalFormat.from_string(extract_p("properties.date", d, None)),
            keywords=[OwcCategory.from_dict(do) for do in extract_p("properties.categories", d, [])],
            resource_metadata=[OwcLink.from_dict(do) for do in extract_p("properties.links.via", d, [])],
            content_description=[OwcLink.from_dict(do) for do in extract_p("properties.links.alternates", d, [])],
            preview=[OwcLink.from_dict(do) for do in extract_p("properties.links.previews", d, [])],
            content_by_ref=[OwcLink.from_dict(do) for do in extract_p("properties.links.data", d, [])],
            offerings=[OwcOffering.from_dict(do) for do in extract_p("properties.offerings", d, [])],
            active=extract_p("properties.active", d, None),
            min_scale_denominator=try_float(extract_p("properties.minscaledenominator", d, None)),
            max_scale_denominator=try_float(extract_p("properties.maxscaledenominator", d, None)),
...
class OwcCreator(object):
    ...
    * Creator/Display and Creator/Application are instantiated as completely
    ...
    def __init__(self, creator_application=None, creator_display=None):
        """
        constructor:
...
class OwcCreatorApplication(object):
    ...
    def __init__(self, title, uri=None, version=None):
        """
        constructor:
    ...
    def to_dict(self):
        return {"title": self.title, "uri": self.uri, "version": self.version}
    ...
    def from_dict(cls, d):
        return OwcCreatorApplication(
            title=extract_p("title", d, None), uri=extract_p("uri", d, None), version=extract_p("version", d, None)
        )
...
class OwcCreatorDisplay(object):
    ...
    def to_dict(self):
        return {"pixelWidth": self.pixel_width, "pixelHeight": self.pixel_height, "mmPerPixel": self.mm_per_pixel}
...
class OwcCategory(object):
    ...
    def to_dict(self):
        return {"scheme": self.scheme, "term": self.term, "label": self.label}
    ...
    def from_dict(cls, d):
        return OwcCategory(
            term=extract_p("term", d, None), scheme=extract_p("scheme", d, None), label=extract_p("label", d, None)
        )
...
class OwcAuthor(object):
    ...
    def to_dict(self):
        return {"name": self.name, "email": self.email, "uri": self.uri}
    ...
    def from_dict(cls, d):
        return OwcAuthor(
            name=extract_p("name", d, None), email=extract_p("email", d, None), uri=extract_p("uri", d, None)
        )
...
class OwcOffering(object):
    ...
    * + extension :Any [0..*]
    ...
    def __init__(self, offering_code, operations=[], contents=[], styles=[]):
        """
        constructor:
    ...
    def to_dict(self):
        return {
            ...
            "operations": [] if len(self.operations) <= 0 else [obj.to_dict() for obj in self.operations],
            "contents": [] if len(self.contents) <= 0 else [obj.to_dict() for obj in self.contents],
            "styles": [] if len(self.styles) <= 0 else [obj.to_dict() for obj in self.styles],
        }
    ...
    def from_dict(cls, d):
        return OwcOffering(
            ...
            operations=[OwcOperation.from_dict(do) for do in extract_p("operations", d, [])],
            contents=[OwcContent.from_dict(do) for do in extract_p("contents", d, [])],
            styles=[OwcStyleSet.from_dict(do) for do in extract_p("styles", d, [])],
        )
...
class OwcOperation(object):
    ...
    * + extension :Any [0..*]
    ...
    def __init__(self, operations_code, http_method, request_url, mimetype=None, request=None, result=None):
        """
        constructor:
...
class OwcStyleSet(object):
    ...
    * + extension :Any [0..*]
    ...
    def __init__(self, name, title, subtitle=None, is_default=None, legend_url=None, content=None):
        """
        constructor:
...
class OwcContent(object):
    ...
    * + extension :Any [0..*]
    ...
    def __init__(self, mimetype, content, url=None, title=None):
        """
        constructor:
    ...
    def to_dict(self):
        return {"type": self.mimetype, "url": self.url, "content": self.content, "title": self.title}
