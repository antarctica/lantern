...
from scar_add_metadata_toolbox.hazmat.owslib.swe.common import Quantity
from dateutil import parser
from scar_add_metadata_toolbox.hazmat.owslib.swe.observation.om import OM_Observation, Result


def get_namespaces():
    ns = Namespaces()
    return ns.get_namespaces(["swe20", "xlink", "sos20", "om20", "gml32", "xsi", "wml2"])
...
class TimeValuePair(object):
    ...
    def __init__(self, element):
        date_str = testXMLValue(element.find(nspv("wml2:MeasurementTVP/wml2:time")))
        try:
            self.datetime = parser.parse(date_str)
        except Exception:
            raise ValueError("Error parsing datetime string: %s" % date_str)

        value_str = testXMLValue(element.find(nspv("wml2:MeasurementTVP/wml2:value")))
        try:
...
class TVPMetadata(object):
    def __init__(self, element):
        ...
        - relatedObservation
        """
        self.quality = testXMLAttribute(element.find(nspv("wml2:quality")), nspv("xlink:href"))
        self.nilReason = testXMLAttribute(element.find(nspv("wml2:nilReason")), nspv("xlink:href"))
        self.comment = testXMLValue(element.find(nspv("wml2:comment")))
        self.qualifier = testXMLAttribute(element.find(nspv("wml2:qualifier")), nspv("xlink:href"))
        self.processing = testXMLValue(element.find(nspv("wml2:processing")), nspv("xlink:href"))
        self.source = testXMLValue(element.find(nspv("wml2:source")), nspv("xlink:href"))
...
class TVPMeasurementMetadata(TVPMetadata):
    ...
    def __init__(self, element):
        ...
        self.uom = testXMLAttribute(element.find(nspv("wml2:uom")), "code")
        self.interpolationType = testXMLAttribute(element.find(nspv("wml2:interpolationType")), nspv("xlink:href"))
        self.censoredReason = testXMLAttribute(element.find(nspv("wml2:censoredReason")), "xlink:href")
