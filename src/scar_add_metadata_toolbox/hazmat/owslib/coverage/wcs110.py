...
class WebCoverageService_1_1_0(WCSBase):
    ...
    def __init__(self, url, xml, cookies, auth=None):
        ...
        # serviceProvider
        elem = self._capabilities.find(self.ns.OWS("ServiceProvider")) or self._capabilities.find(
    ...
    def __init__(self, elem, nmSpc):
        ...
        self.formatOptions = [
            ...
            for f in elem.findall(
                nmSpc.WCS_OWS("Parameter") + "/" + nmSpc.WCS_OWS("AllowedValues") + "/" + nmSpc.WCS_OWS("Value")
            )
        ]  # noqa
        methods = []
