    ...
    def __init__(self, md=None):
        ...
        if md is None:
            ...
            self.charset = _testCodeListValue(
                md.find(util.nspath_eval("gmd:characterSet/gmd:MD_CharacterSetCode", namespaces))
            )

            self.hierarchy = _testCodeListValue(
                md.find(util.nspath_eval("gmd:hierarchyLevel/gmd:MD_ScopeCode", namespaces))
            )
...
class PT_Locale(object):
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            self.languagecode = md.find(util.nspath_eval("gmd:languageCode/gmd:LanguageCode", namespaces)).attrib.get(
                "codeListValue"
            )
            self.charset = md.find(
                util.nspath_eval("gmd:characterEncoding/gmd:MD_CharacterSetCode", namespaces)
...
class CI_ResponsibleParty(object):
    ...
    def __init__(self, md=None):
        ...
        if md is None:
            ...
            val = md.find(
                util.nspath_eval("gmd:contactInfo/gmd:CI_Contact/gmd:onlineResource/gmd:CI_OnlineResource", namespaces)
            )
            if val is not None:
...
class MD_DataIdentification(object):
    ...
    def __init__(self, md=None, identtype=None):
        if md is None:
            ...
            val = md.find(
                util.nspath_eval("gmd:citation/gmd:CI_Citation/gmd:alternateTitle/gco:CharacterString", namespaces)
            )
            ...
            _values += md.findall(
                util.nspath_eval("gmd:resourceConstraints/gmd:MD_Constraints/gmd:useLimitation/gmx:Anchor", namespaces)
            )
            for i in _values:
                val = util.testXMLValue(i)
            ...
            self.uom = []
            for i in md.findall(
                util.nspath_eval("gmd:spatialResolution/gmd:MD_Resolution/gmd:distance/gco:Distance", namespaces)
            ):
                val = util.testXMLValue(i)
                if val is not None:
            ...
            for val in md.findall(
                util.nspath_eval("gmd:spatialRepresentationType/gmd:MD_SpatialRepresentationTypeCode", namespaces)
            ):
            ...
            for i in md.findall(util.nspath_eval("gmd:descriptiveKeywords", namespaces)):
                ...
                mdkw["type"] = _testCodeListValue(
                    i.find(util.nspath_eval("gmd:MD_Keywords/gmd:type/gmd:MD_KeywordTypeCode", namespaces))
                )
            ...
            for extent in extents:
                if val is None:
                    for e in extent.findall(util.nspath_eval("gmd:EX_Extent/gmd:geographicElement", namespaces)):
                        if (
                            e.find(util.nspath_eval("gmd:EX_GeographicBoundingBox", namespaces)) is not None
                            or e.find(util.nspath_eval("gmd:EX_BoundingPolygon", namespaces)) is not None
                        ):
                            val = e
                            break
...
class MD_Distributor(object):
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            self.contact = None
            val = md.find(
                util.nspath_eval("gmd:MD_Distributor/gmd:distributorContact/gmd:CI_ResponsibleParty", namespaces)
            )
            if val is not None:
...
class MD_Distribution(object):
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            val = md.find(
                util.nspath_eval("gmd:distributionFormat/gmd:MD_Format/gmd:name/gco:CharacterString", namespaces)
            )
            self.format = util.testXMLValue(val)

            val = md.find(
                util.nspath_eval("gmd:distributionFormat/gmd:MD_Format/gmd:version/gco:CharacterString", namespaces)
            )
...
class SV_ServiceIdentification(object):
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            self.couplingtype = _testCodeListValue(
                md.find(util.nspath_eval("gmd:couplingType/gmd:SV_CouplingType", namespaces))
            )
            ...
            for i in md.findall(util.nspath_eval("srv:containsOperations", namespaces)):
                tmp = {}
                val = i.find(
                    util.nspath_eval("srv:SV_OperationMetadata/srv:operationName/gco:CharacterString", namespaces)
                )
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            self.function = _testCodeListValue(
                md.find(util.nspath_eval("gmd:function/gmd:CI_OnLineFunctionCode", namespaces))
            )
...
class MD_ReferenceSystem(object):
    ...
    def __init__(self, md=None):
        if md is None:
            ...
            if val is None:
                val = md.find(
                    util.nspath_eval("gmd:referenceSystemIdentifier/gmd:RS_Identifier/gmd:code/gmx:Anchor", namespaces)
                )
            if val is not None:
