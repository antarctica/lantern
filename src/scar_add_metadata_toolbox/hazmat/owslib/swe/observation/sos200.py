...
class SensorObservationService_2_0_0(object):
    ...
    def describe_sensor(self, outputFormat=None, procedure=None, method=None, **kwargs):
        ...
        response = openURL(
            base_url, request, method, username=self.username, password=self.password, **url_kwargs
        ).read()
...
class SOSGetObservationResponse(object):
    ...
    def __init__(self, element):
        obs_data = element.findall(nspath_eval("sos:observationData/om20:OM_Observation", namespaces))
        self.observations = []
