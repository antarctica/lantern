...
def force900913(epsg):
    ...
    if FORCE900913 and epsg.upper() in ["OSGEO:41001", "EPSG:3785", "EPSG:3857", "EPSG:54004"]:
        return "EPSG:900913"
    else:
        return epsg
...
class TileMapService(object):
    ...
    def _getcapproperty(self):
        ...
        if not self._capabilities:
            reader = TMSCapabilitiesReader(self.version, url=self.url, un=self.username, pw=self.password)
    ...
    def gettile(self, x, y, z, id=None, title=None, srs=None, mimetype=None, timeout=None):
        ...
        if id:
            return self._gettilefromset(
                self.contents[id].tilemap.tilesets, x, y, z, self.contents[id].tilemap.extension, timeout=timeout
            )
                ...
                if tm.title == title and tm.srs == srs:
                    if mimetype:
                        if tm.tilemap.mimetype == mimetype:
                            return self._gettilefromset(
                                tm.tilemap.tilesets, x, y, z, tm.tilemap.extension, timeout=timeout
                            )
                    else:
    ...
    def _parse(self, elem):
        ...
        if ts is not None:
            ...
            for tileset in tilesets:
                ...
                self.tilesets.append({"href": href, "units-per-pixel": upp, "order": order})
