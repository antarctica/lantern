...
def dict_union(d1, d2):
    return dict(
        (x, (dict_union(d1.get(x, {}), d2[x]) if isinstance(d2.get(x), dict) else d2.get(x, d1.get(x))))
...
def clean_ows_url(url):
    ...
    newurl = urlunparse(
        [parsed.scheme, parsed.netloc, parsed.path, parsed.params, urlencode(filtered_kvp, doseq=True), parsed.fragment]
    )
    ...
    for param in param_list:
        if len(param) > 2:
            if not is_number(param[1]):
                string += "&" + urlencode(
                    {param_name: param[0] + '("' + makeString(param[1]) + '","' + makeString(param[2]) + '")'}
                )  # noqa
            else:
                string += "&" + urlencode(
                    {param_name: param[0] + "(" + makeString(param[1]) + "," + makeString(param[2]) + ")"}
...
class Authentication(object):
    ...
    def __init__(self, token=None, username=None, password=None, cert=None, verify=True, shared=False):
        """
