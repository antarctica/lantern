...
class WebMapTileService(object):
    ...
    def __init__(
        ...
        reader = WMTSCapabilitiesReader(self.version, url=self.url, headers=self.headers, auth=self.auth)
        ...
        **kwargs : extra arguments
        ...
        if self.restonly:
            resurl = self.buildTileResource(
                layer, style, format, tilematrixset, tilematrix, row, column, **vendor_kwargs
            )
            u = openURL(resurl, headers=self.headers, auth=self.auth, timeout=self.timeout)
            return u

        # KVP implemetation
        data = self.buildTileRequest(layer, style, format, tilematrixset, tilematrix, row, column, **vendor_kwargs)

        if base_url is None:
            base_url = self.url
            try:
                methods = self.getOperationByName("GetTile").methods
                get_verbs = [x for x in methods if x.get("type").lower() == "get"]
                if len(get_verbs) > 1:
...
class TileMatrixSetLink(object):
    ...
    def from_elements(link_elements):
        ...
        for link_element in link_elements:
            ...
            if len(matrix_set_elements) == 0:
                ...
                set_limits_elements = link_element.findall(_TILE_MATRIX_SET_LIMITS_TAG)
                if set_limits_elements:
                    raise ValueError(
                        "Multiple instances of TileMatrixSet" " plus TileMatrixSetLimits in %s" % link_element
                    )
                ...
                for limits_element in link_element.findall(path):
                    ...
                    if tml.tilematrix:
                        if tml.tilematrix in tilematrixlimits:
                            msg = 'TileMatrixLimits with tileMatrix "%s" ' "already exists" % tml.tilematrix
    ...
    def __repr__(self):
        fmt = "<TileMatrixSetLink: {self.tilematrixset}" ", tilematrixlimits={{...}}>"
        return fmt.format(self=self)
...
class ContentMetadata:
    ...
    def __init__(self, elem, parent=None, index=0, parse_remote_metadata=False):
        ...
        self._tilematrixsets = [
            f.text.strip() for f in elem.findall(_TILE_MATRIX_SET_LINK_TAG + "/" + _TILE_MATRIX_SET_TAG)
        ]
        ...
        for s in elem.findall(_STYLE_TAG):
            ...
            keywords = [f.text for f in s.findall(_KEYWORDS_TAG + "/" + _KEYWORD_TAG)]
        ...
        self.keywords = [f.text for f in elem.findall(_KEYWORDS_TAG + "/" + _KEYWORD_TAG)]
