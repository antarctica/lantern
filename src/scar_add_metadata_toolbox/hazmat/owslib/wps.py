...
def _fix_auth(auth, username=None, password=None, verify=None, cert=None):
    ...
    if any(p is not None for p in (username, password, verify, cert)):
        message = (
            'The use of "username", "password", "verify", and "cert" is deprecated. '
...
class WebProcessingService(object):
    ...
    def getcapabilities(self, xml=None):
        ...
        if xml:
            ...
            self._capabilities = reader.readFromUrl(self.url, headers=self.headers)
...
class WPSReader(object):
    ...
    def _readFromUrl(
        self, url, data, timeout, method="Get", username=None, password=None, headers=None, verify=True, cert=None
    ):
        """
...
class WPSCapabilitiesReader(WPSReader):
    ...
    def readFromUrl(self, url, username=None, password=None, headers=None, verify=None, cert=None):
        """
...
class WPSDescribeProcessReader(WPSReader):
    ...
    def readFromUrl(self, url, identifier, username=None, password=None, headers=None, verify=None, cert=None):
        """
...
class WPSExecuteReader(WPSReader):
    ...
    def readFromUrl(
        self, url, data={}, method="Get", username=None, password=None, headers=None, verify=None, cert=None
    ):
        """
...
class WPSExecution(object):
    ...
    def buildRequest(self, identifier, inputs=[], output=None, mode=ASYNC, lineage=False):
        ...
        root.set(
            nspath_eval("xsi:schemaLocation", namespaces), "%s %s" % (namespaces["wps"], WPS_DEFAULT_SCHEMA_LOCATION)
        )
        ...
        if output is not None:
            ...
            responseDocumentElement = etree.SubElement(
                ...
                attrib={
                    "storeExecuteResponse": str(_async).lower(),
            ...
            if isinstance(output, str):
                ...
                for ouputTuple in output:
                    ...
                    self._add_output(responseDocumentElement, identifier, asReference=as_reference, mimeType=mime_type)
            else:
    ...
    def _add_output(self, element, identifier, asReference=None, mimeType=None):
        output_element = etree.SubElement(element, nspath_eval("wps:Output", namespaces))
        ...
        etree.SubElement(output_element, nspath_eval("ows:Identifier", namespaces)).text = identifier
    ...
    def checkStatus(self, url=None, response=None, sleepSecs=60):
        ...
        if response is None:
            ...
            try:
                response = reader.readFromUrl(self.statusLocation, headers=self.headers)
            except Exception:
    ...
    def submitRequest(self, request):
        ...
        response = reader.readFromUrl(self.url, request, method="Post", headers=self.headers)
...
class InputOutput(object):
    ...
    def _parseData(self, element):
        ...
        complex_data_element = element.find(nspath("ComplexData", ns=getNamespace(element)))
        ...
        if literal_data_element is not None:
            ...
            for sub_element in literal_data_element:
                ...
                if sub_element.tag.endswith("DefaultValue"):
                    self.defaultValue = getTypedValue(self.dataType, sub_element.text)
    ...
    def _parseComplexData(self, element, complexDataElementName):
        ...
        if complex_data_element is not None:
            ...
            for supported_comlexdata_element in complex_data_element.findall("SupportedComplexData"):
                self.supportedValues.append(
                    ComplexData(
                        mimeType=testXMLValue(supported_comlexdata_element.find("Format")),
                        encoding=testXMLValue(supported_comlexdata_element.find("Encoding")),
                        schema=testXMLValue(supported_comlexdata_element.find("Schema")),
                    )
                )

            for format_element in complex_data_element.findall("Supported/Format"):
                self.supportedValues.append(
                    ComplexData(
            ...
            if default_format_element is not None:
                self.defaultValue = ComplexData(
                    mimeType=testXMLValue(default_format_element.find("MimeType")),
                    encoding=testXMLValue(default_format_element.find("Encoding")),
...
class Output(InputOutput):
    ...
    def retrieveData(self, username=None, password=None, headers=None, verify=True, cert=None):
        ...
        if "?" in url:
            ...
            u = openURL(
                url, "", method="Get", username=username, password=password, headers=headers, verify=verify, cert=cert
            )
    ...
    def writeToDisk(self, path=None, username=None, password=None, headers=None, verify=True, cert=None):
        """
...
class BoundingBoxDataInput(object):
    ...
    def get_xml(self):
        ...
        bbox_el = etree.SubElement(data_el, nspath_eval("wps:BoundingBoxData", namespaces), attrib=attrib)
        lc_el = etree.SubElement(bbox_el, nspath_eval("ows:LowerCorner", namespaces))
        lc_el.text = "{0[0]} {0[1]}".format(self.lower_corner)
        uc_el = etree.SubElement(bbox_el, nspath_eval("ows:UpperCorner", namespaces))
...
class WFSFeatureCollection(FeatureCollection):
    ...
    def getXml(self):
        root = etree.Element(
            nspath_eval("wps:Reference", namespaces), attrib={nspath_eval("xlink:href", namespaces): self.url}
        )
        if self.method:
        ...
        getFeatureElement = etree.SubElement(
            bodyElement,
            nspath_eval("wfs:GetFeature", namespaces),
            attrib={
                "service": "WFS",
                "version": "1.1.0",
    ...
    def getXml(self):
        ...
        if len(self.filters) > 0:
            ...
            for filter in self.filters:
                ...
                etree.SubElement(
                    filterElement,
                    nspath_eval("ogc:GmlObjectId", namespaces),
...
def printInputOutput(value, indent=""):
    ...
    print(
        (
            "{} identifier={}, title={}, abstract={}, data type={}".format(
    ...
    if isinstance(value, Input):
        print(("{} minOccurs={}, maxOccurs={}".format(indent, value.minOccurs, value.maxOccurs)))

    # Output fields
    if isinstance(value, Output):
        print(("{} reference={}, mimeType={}".format(indent, value.reference, value.mimeType)))
