...
def _init_store(logger: logging.Logger, config: Config, cached: bool = False) -> GitLabStore | GitLabCachedStore:
    """Initialise store."""
    source = GitLabSource(
        endpoint=config.STORE_GITLAB_ENDPOINT,
        project=config.STORE_GITLAB_PROJECT_ID,
        ref=config.STORE_GITLAB_DEFAULT_BRANCH,
    )
    store = GitLabStore(logger=logger, source=source, access_token=config.STORE_GITLAB_TOKEN)

    if not cached:
        return store

    return GitLabCachedStore.from_gitlab_store(
        store=store,
...
def _stage0(logger: logging.Logger, config: Config, working_path: Path) -> None:
    ...
    store = _init_store(logger=logger, config=config)
...
def _stage1(logger: logging.Logger, config: Config, working_path: Path) -> None:
    ...
    default_author_name, default_author_email = get_default_author()
    answers = inquirer.prompt(
        [
            inquirer.Text(name="author_name", message="Committer name", default=default_author_name),
            inquirer.Text(name="author_email", message="Committer email", default=default_author_email),
        ]
    )
...
def _stage2(logger: logging.Logger, config: Config) -> None:
    ...
    if config.STORE_GITLAB_DEFAULT_BRANCH != "main":
